...
class trtYOLO():
    ...
    def detect_path(self,target_path):
        ...
        img_ = torch.from_numpy(img_).float().div(255.0).unsqueeze(0)
        img_ = img_.numpy()
        print("[shape]input shape:",img_.shape)
        im_time2 = time.time()
        ...
        # print(boxes,clss,clss_prob)
        return (boxes,clss,clss_prob)
    ...
    def detect_frame(self,frame):
        ...
        new_width, new_height = self.img_size()
        pad_h, pad_w, ratio = calculate_padding(h, w, new_height, new_width)
        constant = cv2.copyMakeBorder(frame, pad_h, pad_h, pad_w, pad_w, cv2.BORDER_CONSTANT, value=0)#top,bottom,left,right
        img_ = cv2.resize(constant,(new_height,new_width), interpolation=cv2.INTER_CUBIC)
        # cv2.imshow('constant',constant)
        # while True:
        #     if cv2.waitKey(1) & 0xFF == ord('z'):       # 按q退出
        ...
        if main_box_corner == None:
            return    main_box_corner,clss,clss_prob
        boxes = torch.empty(size = main_box_corner.shape)
    ...
    def detection(self,img):
        ...
        with torch.no_grad():
            ...
            for detections in outputs:
                # print("detections device:",detections.device)
    ...
    def detect_frame_batch(self, frame_dic_list):
